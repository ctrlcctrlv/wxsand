...
MainFrame::MainFrame(const wxString& title, const wxPoint& pos, const wxSize& size, 
  ...
  wxBoxSizer* sizer = new wxBoxSizer(wxHORIZONTAL);
  {
	...
	ctrl->SetValue(22);
      ...
      {
	RH2->Add(new wxRadioButton(this, 1010, _("1")), 0, wxCENTER | wxLEFT, 2);
	RH2->Add(new wxRadioButton(this, 1011, _("2")), 0, wxCENTER | wxLEFT, 2);
	wxRadioButton* rb4 = new wxRadioButton(this, 1012, _("4"));
	RH2->Add(rb4, 0, wxCENTER | wxLEFT, 2);
	rb4->SetValue(true);
	RH2->Add(new wxRadioButton(this, 1013, _("8")), 0, wxCENTER | wxLEFT, 2);
	RH2->Add(new wxRadioButton(this, 1014, _("16")), 0, wxCENTER | wxLEFT, 2);
	RH2->Add(new wxRadioButton(this, 1015, _("32")), 0, wxCENTER | wxLEFT, 2);
	RH2->Add(new wxRadioButton(this, 1017, _("128")), 0, wxCENTER | wxLEFT, 2);
...
void MainFrame::loadPhysics(wxString filename){
  ...
  if (version == 1){
	...
	if (strcmp(type, "element") == 0){
	  ...
	  for(int k=0;k<100;++k){
	    colors[elementNumber][k] = wxColour((unsigned char)red, 
						(unsigned char)green, 
						(unsigned char)blue);
	  }
	  ...
	  if (n_element == -1){
	    loadFileError(lineNumber, _("Invalid hotcolor line. Unknown element."));
	    fclose(file);
	    return;
	  }
	  ...
	  for(int k=1;k<100;++k){
	    ...
	    colors[n_element][k] = wxColour((unsigned char)nred, 
	  ...
	  if (n_center == -1){
	    loadFileError(lineNumber, _("Invalid hotself line. Unknown element."));
	    fclose(file);
	    return;
	  }
	  ...
	  if (n_center == -1 || n_neighbor == -1){
	    loadFileError(lineNumber, _("Invalid hotneighbor line. Unknown element."));
	    fclose(file);
	    return;
	  }
	  ...
	  if (gar == NULL || center == NULL || neighbor == NULL || diameter == NULL || pelement == NULL){
	  ...
	  if (n_center == -1 || n_neighbor == -1 || n_pelement == -1){
	    loadFileError(lineNumber, _("Invalid pexplosion line. Unknown element."));
	    fclose(file);
	    return;
	  }
	  ...
	  if (gar == NULL || center == NULL || neighbor == NULL || xsp == NULL || ysp == NULL){
	  ...
	  if (n_center == -1 || n_neighbor == -1){
	    loadFileError(lineNumber, _("Invalid explosion line. Unknown element."));
	    fclose(file);
	    return;
	  }
