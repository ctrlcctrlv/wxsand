...
Canvas::Canvas(wxWindow* parent, wxWindowID id = -1,
...
inline void found(int center, int other, int cx, int cy, int ox, int oy) {
  ...
  drawData(other);
  calc[center] = true;
  if (data[other] != 0) calc[other] = true;
}
...
inline void checkForTransformation(int center, int other, int cx, int cy,
                                   int ox, int oy) {
    ...
    if (data[other] != 0) {
      ...
      if (c > 0) {
        if (energy[other] > energy[center]) {
          ...
          if (energy[center] > 99) energy[center] = 99;
  ...
  if (doWalls &&
      (ox == 0 || ox == g_width - 1 || oy == 0 || oy == g_height - 1))
    return;
  ...
  if (rand() < ctrans_prob[t1][t2][energy[center] + energy[other]]) {
    ...
    if (trans_xspeed[t1][t2] != 0) {
      xspeed[center] =
          (int)round(trans_xspeed[t1][t2] * double(rand()) / RAND_MAX);
      if (rand() < RAND_MAX / 2) xspeed[center] = -xspeed[center];

      xspeed[other] =
          (int)round(trans_xspeed[t1][t2] * double(rand()) / RAND_MAX);
      if (rand() < RAND_MAX / 2) xspeed[other] = -xspeed[other];
    }

    if (trans_yspeed[t1][t2] != 0) {
      yspeed[center] =
          (int)round(trans_yspeed[t1][t2] * double(rand()) / RAND_MAX);
      if (rand() < RAND_MAX / 2) yspeed[center] = -yspeed[center];
      yspeed[other] =
          (int)round(trans_yspeed[t1][t2] * double(rand()) / RAND_MAX);
      if (rand() < RAND_MAX / 2) yspeed[other] = -yspeed[other];
    }
  ...
  for (int y = g_height - 2; y > 0; --y) {
    ...
    for (int x = sx; x != ex; x += dx) {
      ...
      if (rand() < cdeath_prob[data[center]][energy[center]]) {
        ...
        if (data[center] == 0) energy[center] = 0;
        drawData(center);
        continue;
      }
      ...
      if (doEnergy) {
        if (conductivity[data[center]] > 1.0) {
          int e = energy[center] +
                  (int)round(99.0 * (conductivity[data[center]] - 1.0));
          if (e > 99) e = 99;
          energy[center] = (char)e;
          drawData(center);
        }
        if (rand() < conductivity[data[center]] * 0.1 * RAND_MAX) {
          if (data[left] == 0 || data[right] == 0 || data[up] == 0 ||
              data[down] == 0 || data[upleft] == 0 || data[upright] == 0 ||
              data[downleft] == 0 || data[downright] == 0) {
      ...
      if (doGravity) {
        if (gravity[data[center]] == 0) continue;

        if (yspeed[center] < 0) continue;
            ...
            if (!doWalls || (y < g_height - 2 && x > 1)) {
              ...
              if (data[downleft] == 0) {
                ...
                if (density[data[center]] > density[data[downleft]]) {
                  if (rand() <
                      ccogravitydensity[data[center]][data[downleft]]) {
                    found(center, downleft, x, y, x - 1, y + 1);
                    continue;
                  }
                }
            ...
            if (!doWalls || (y < g_height - 2 && x < g_width - 2)) {
              ...
              if (data[downright] == 0) {
                ...
                if (density[data[center]] > density[data[downright]]) {
                  if (rand() <
                      ccogravitydensity[data[center]][data[downright]]) {
                    found(center, downright, x, y, x + 1, y + 1);
                    continue;
                  }
                }
...
void Canvas::OnMouseLeftUp(wxMouseEvent& event) { mouseIsDown = false; }
